...
Task creation supports `--idempotency-key` for agent retry safety. When the same key is reused with the same create payload, the CLI returns the cached task instead of creating a duplicate remote task. Reusing the same key with different title/project/content/due/priority arguments fails validation; generate a new key whenever the intended task changes. Keys are stored locally in `idempotency.json` next to the active config directory.
